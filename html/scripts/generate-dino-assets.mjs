...
const SILHOUETTE_OUT = resolve(
  __dirname,
  "../examples/dino-drop/src/dinoSilhouetteAuto.ts"
);
...
function drawBackDetail(ctx) {
  // Highlight + skin spots get clipped to the body path so they never extend
  // past the silhouette — important for the top-edge column scan we use to
  // build the physics collider.
  ctx.save();
  bodyPath(ctx);
  ctx.clip();
...
//
// Walks each column of a canvas and records the first opaque pixel. Gives a
// per-column top-edge polyline that traces the visible silhouette exactly.
// Then collapses long flat runs with Ramer–Douglas–Peucker so the resulting
// Matter.js collider isn't ~1200 vertices.

function extractTopEdge(cv, alphaThreshold = 32) {
  const ctx = cv.getContext("2d");
...
// Per-column min across two dense top-edge scans. When both PNGs have an
// opaque pixel at column x, the smaller y wins — that's the pixel that's
// actually visible in the composited scene. This is what the static collider
// needs to follow.
function mergeTopEdges(a, b) {
  const byX = new Map();
  for (const [x, y] of a) byX.set(x, y);
  for (const [x, y] of b) {
    const existing = byX.get(x);
    if (existing === undefined || y < existing) byX.set(x, y);
  }
  return [...byX.entries()]
    .sort(([ax], [bx]) => ax - bx)
    .map(([x, y]) => [x, y]);
...
function formatPolyline(pts) {
  return pts.map(([x, y]) => `  [${x}, ${y}],`).join("\n");
}

function writeSilhouetteModule(bodyEdge, tailEdge, mergedEdge, bodyCv, tailCv) {
  const body = `[\n${formatPolyline(bodyEdge)}\n]`;
  const tail = `[\n${formatPolyline(tailEdge)}\n]`;
  const merged = `[\n${formatPolyline(mergedEdge)}\n]`;
  const out = `// AUTO-GENERATED by scripts/generate-dino-assets.mjs — do not edit by hand.
...
export const BODY_TOP_EDGE: ReadonlyArray<readonly [number, number]> = ${body};

export const TAIL_TOP_EDGE: ReadonlyArray<readonly [number, number]> = ${tail};

export const MERGED_TOP_EDGE: ReadonlyArray<readonly [number, number]> = ${merged};

export const BODY_SIZE = { width: ${bodyCv.width}, height: ${bodyCv.height} } as const;
export const TAIL_SIZE = { width: ${tailCv.width}, height: ${tailCv.height} } as const;
`;
  writeFileSync(SILHOUETTE_OUT, out);
}
...
const bodyCv = drawBrontoBody();
const tailCv = drawBrontoTail();
writePng(bodyCv, "brontosaurus-body.png");
writePng(tailCv, "brontosaurus-tail.png");

const RDP_EPSILON = 1.5;
const bodyDense = extractTopEdge(bodyCv);
const tailDense = extractTopEdge(tailCv);
const mergedDense = mergeTopEdges(bodyDense, tailDense);
const bodyEdge = rdp(bodyDense, RDP_EPSILON);
const tailEdge = rdp(tailDense, RDP_EPSILON);
const mergedEdge = rdp(mergedDense, RDP_EPSILON);
writeSilhouetteModule(bodyEdge, tailEdge, mergedEdge, bodyCv, tailCv);
...
console.log(
  `wrote dinoSilhouetteAuto.ts (body: ${bodyEdge.length} pts, tail: ${tailEdge.length} pts, merged: ${mergedEdge.length} pts) to ${SILHOUETTE_OUT}`
);
